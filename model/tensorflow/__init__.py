    ...
    def __init__(
        ...
        **kwargs
        ...
        for i, units in enumerate(att_hidden_units):
            att_layers.append(layers.Dense(units, use_bias=use_bias,
                                           name=f'att_fc_{i}'))
            if att_activation == 'dice':
                ...
                att_layers.append(layers.Activation(att_activation,
                                                     name=f'att_act_{i}'))
...
class DIN(Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # ── Local activation unit ────────────────────────────────────────────
        self.attention = LocalActivationUnit(att_hidden_units, att_activation, bias,
        ...
        for i, units in enumerate(dnn_hidden_units):
            dnn_layers.append(layers.Dense(units, use_bias=bias,
                                           name=f'dnn_fc_{i}'))
            if dnn_use_bn:
            ...
            if dnn_activation == 'dice':
                ...
                dnn_layers.append(layers.Activation(dnn_activation,
                                                     name=f'dnn_act_{i}'))
            if dropout > 0.0:
    ...
    def call(self, inputs, training: bool = False) -> tf.Tensor:
        ...
        target_flat   = tf.reshape(target_embs,
                                   [-1, self.num_target_features * self.dim_emb])
        dense_flat    = tf.reshape(dense_embs,
